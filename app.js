...
async function callAI(words, settings) {
    ...
    // 随机选择故事风格，增加趣味性
    const storyStyles = [
        '一个充满悬念的侦探故事，主角需要破解一个谜团',
        '一个温馨搞笑的日常故事，发生在咖啡馆或学校',
        '一个奇幻冒险故事，主角意外进入了魔法世界',
        '一个科幻故事，发生在未来的太空站或外星球',
        '一个动物拟人化的童话故事，动物们有自己的社会',
        '一个穿越时空的冒险，主角回到了古代或未来',
        '一个关于美食的故事，主角是厨师或美食评论家',
        '一个体育竞技故事，关于一场重要的比赛',
        '一个关于友情的感人故事，朋友们一起克服困难'
    ];
    const randomStyle = storyStyles[Math.floor(Math.random() * storyStyles.length)];
    
    const prompt = `你是一个才华横溢的故事作家，擅长创作引人入胜的短篇故事。

请创作${randomStyle}，自然融入以下所有单词：

单词列表：${words.join(', ')}

写作要求：
1. 故事要有明确的主角、场景和情节冲突
2. 情节要有"起承转合"：开头引入 → 发展铺垫 → 高潮转折 → 结局收尾
3. 加入至少一个出人意料的转折或幽默元素
4. 所有单词必须自然融入对话或叙述中，不能生硬堆砌
5. 每个单词用 **word** 格式标记
6. 故事长度200-350词，要完整精彩

格式要求：
[STORY]
(纯英文故事，单词用**word**标记，禁止出现任何中文)
